...
def start_scanner(placeholder, confidence_threshold):
    ...
    # 1. LOAD MODELS (Ibinalik sa LBPH Local Recognizer)
    try:
        # Siguraduhing may trainer.yml bago mag-load
        if not os.path.exists("trainer.yml"):
            st.error("❌ Error: 'trainer.yml' hindi matagpuan! Patakbuhin muna ang train.py sa iyong computer.")
            return False
    ...
    # =====================================================
    # CONFIGURATION BASED ON FOLDERS
    # =====================================================
    names = [
        "None",             # ID 0
        "Hazel Mae",        # ID 1
    ...
    while st.session_state.get("scanner_running", True):
        ...
        for (x, y, w, h) in faces:
            ...
            if do_predict:
                ...
                # 2. LBPH Local Prediction Execution
                try:
                    # Ang LBPH ay nangangailangan ng grayscale crop region
                    face_roi_gray = gray[y:y+h, x:x+w]
                    
                    id, confidence = recognizer.predict(face_roi_gray)
                    
                    # Sa LBPH, ang mas MABABANG confidence score ay nangangahulugang mas MATCH ang mukha.
                    # Kaya kino-convert natin ito sa percentage (0-100%) kung saan mas mataas, mas kilala.
                    ...
                    if match_perc >= confidence_threshold:
                        ...
                        st.session_state.last_result = f"Unknown ({match_perc}%)"
                        st.session_state.last_color = (0, 0, 255) # Red
                except Exception as ex:
                    print(f"❌ LBPH Prediction Error: {ex}")
                    st.session_state.last_result = "Scan Error"
                    st.session_state.last_color = (0, 0, 255)
            else:
